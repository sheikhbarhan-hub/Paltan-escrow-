"""
Paltan Transactions Escrow Bot — Telethon Build (Fixed)
Single-file. Standard inline buttons. Full escrow + admin panel.
"""
...
def main_menu(admin=False):
    rows = [
        [KB("✦ My status", "my_status")],
        [KB("★ My Deals Info", "my_deals")],
        [KB("➤ My Pending Deals", "my_pending")],
        [KB("✓ Escrow Global status", "global_status")],
    ]
    if admin:
        rows.append([KB("🛡 Admin Panel", "admin_panel")])
    return rows


def admin_menu():
    return [
        [KB("📨 SEND VOUCH", "adm_send_vouch"), KB("💼 MANAGE DEALS", "adm_manage_deals")],
        [KB("🏆 MANAGE RANKS", "adm_manage_ranks"), KB("👑 MANAGE SUPER ADMINS", "adm_super_admins")],
        [KB("🛡 MANAGE ADMINS", "adm_admins"), KB("👥 VIEW USERS", "adm_view_users")],
        [KB("🚫 BAN USER", "adm_ban"), KB("✅ UNBAN USER", "adm_unban")],
        [KB("📢 BROADCAST", "adm_broadcast"), KB("📊 API STATS", "adm_stats")],
        [KB("📝 ACTIVITY LOG", "adm_logs"), KB("💸 PRICING PLANS", "adm_pricing")],
        [KB("🎟 REDEEM CODES", "adm_redeem"), KB("➕ ADD CREDITS", "adm_addcredit")],
        [KB("➖ DEDUCT CREDITS", "adm_deductcredit"), KB("➕ ADD CREDITS ALL", "adm_addall")],
        [KB("➖ DEDUCT ALL", "adm_deductall"), KB("🔗 FORCE JOIN", "adm_forcejoin")],
        [KB("⚙️ SETTINGS", "adm_settings"), KB("📜 DEAL HISTORY", "adm_history")],
        [KB("📤 EXPORT DATA", "adm_export"), KB("🔒 PROTECT NUMBER", "adm_protect")],
        [KB("📋 PROTECTED LIST", "adm_protlist"), KB("🎯 TRACK USER", "adm_track")],
        [KB("🔴 TOGGLE FREE MODE", "adm_freemode")],
        [KB("🔄 REFRESH", "adm_refresh")],
        [KB("⬅ Back to Main", "back_main")],
    ]
...
async def on_callback(event):
    try:
        ...
        if data == "my_status":
            ...
            await event.edit(text, parse_mode="html", buttons=main_menu(is_admin(uid)))
            ...
            if not deals:
                ...
                return
            rows = [[KB(d['trade_id'], f"deal_{d['trade_id']}")] for d in deals]
            rows.append([KB("⬅ Back", "back_main")])
            await event.edit("❤ <b>All deals info !</b>\n━━━━━━━━━━━━━━━━━━━━\nSelect the deal below for info :",
            ...
            await event.edit("⚡ <b>Main Menu</b>\nSelect an option below:",
            ...
            if d:
                ...
                await event.edit(text, parse_mode="html",
                                 buttons=[[KB("⬅ Back", "my_deals")]])

        elif data == "admin_panel":
            if not is_admin(uid):
                await event.answer("Access Denied.", alert=True)
                return
            await event.edit("🛡 <b>Admin Control Panel</b>\n━━━━━━━━━━━━━━━━━━━━\nSelect an option:",
...
async def handle_admin(event, uid, data):
    ...
    if data == "adm_view_users":
        ...
        await event.edit(t, parse_mode="html", buttons=admin_menu())
        return
    ...
    if data == "adm_freemode":
        ...
        await event.edit(f"⚙️ Free Mode is now {'🟢 ON' if nv == '1' else '🔴 OFF'}.",
                         parse_mode="html", buttons=admin_menu())
        return
