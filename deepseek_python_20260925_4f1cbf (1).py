...
def btn(text, callback_data):
    # NOTE: Standard Telegram Bot API does NOT support colors on Inline Buttons.
    # Removing 'style' to prevent API crash.
...
def main_menu_kb(is_admin=False):
    rows = [
        [btn("✦ My status", "my_status")],
        [btn("★ My Deals Info", "my_deals")],
        [btn("➤ My Pending Deals", "my_pending")],
        [btn("✓ Escrow Global status", "global_status")],
    ]
    if is_admin:
...
def admin_panel_kb():
    return make_kb([
        [btn("📨 SEND VOUCH", "admin_send_vouch"), btn("💼 MANAGE DEALS", "admin_manage_deals")],
        [btn("🏆 MANAGE RANKS", "admin_manage_ranks"), btn("👑 MANAGE SUPER ADMINS", "admin_manage_super_admins")],
        [btn("🛡 MANAGE ADMINS", "admin_manage_admins"), btn("👥 VIEW USERS", "admin_view_users")],
        [btn("🚫 BAN USER", "admin_ban_user"), btn("✅ UNBAN USER", "admin_unban_user")],
        [btn("📢 BROADCAST", "admin_broadcast"), btn("📊 API STATS", "admin_api_stats")],
        [btn("📝 ACTIVITY LOG", "admin_activity_log"), btn("💸 PRICING PLANS", "admin_pricing_plans")],
        [btn("🎟 REDEEM CODES", "admin_redeem_codes"), btn("➕ ADD CREDITS", "admin_add_credits")],
        [btn("➖ DEDUCT CREDITS", "admin_deduct_credits"), btn("➕ ADD CREDITS ALL", "admin_add_credits_all")],
        [btn("➖ DEDUCT ALL", "admin_deduct_all"), btn("🔗 FORCE JOIN", "admin_force_join")],
        [btn("⚙️ SETTINGS", "admin_settings"), btn("📜 DEAL HISTORY", "admin_sms_history")],
        [btn("📤 EXPORT DATA", "admin_export_script"), btn("🔒 PROTECT NUMBER", "admin_protect_number")],
        [btn("📋 PROTECTED LIST", "admin_protected_list"), btn("🎯 TRACK USER", "admin_track_number")],
        [btn("🔴 TOGGLE FREE MODE", "admin_toggle_free_mode")],
        [btn("🔄 REFRESH", "admin_refresh")],
    ])
    ...
    try:
        ...
        if data == "my_status":
            ...
            if not deals:
                ...
                return
            rows = [[btn(d['trade_id'], f"deal_info_{d['trade_id']}")] for d in deals]
